...
public class ProjectionWindow {
    ...
    private float sizeModifier; //manipulates the size of the objects
    private int fadeModifier;
    ...
    private void SpaceLaunch(int sW, int sH) {
        ...
        inputScrollPane.setSize(sW - 6, sH / 18 - 2);
        inputScrollPane.setLocation(3, sH - sH / 5);
        inputScrollPane.setOpaque(true);
        inputScrollPane.setBackground(new Color(0.2f, 0.2f, 0.2f, 0.3f));

        JScrollPane logScrollPane = new JScrollPane(log);
        logScrollPane.setSize(sW - 6, sH / 7);
        logScrollPane.setLocation(3, sH - sH / 7);
        ...
        projectionFrame.addKeyListener(terminal);
        ...
        ChangePane(log, "Captain, Starscape initialized successfuly...\n Type !help to view available commands!\n Click to execute a command.", StarRed);
    ...
    public void DiscoverAt(int xC, int yC) {
        ...
        switch (inputCommand) {
            ...
            case "!-": {
                if (sizeModifier > 0.4f) {
                    sizeModifier -= 0.2f;
                    if(fadeModifier>1){
                        fadeModifier--;
                    }
                } else{
                    ChangePane(log, "Minimum size already reached, can not go any further...", StarRed);
                }
                break;
            }
            case "!exit": {
                //terminal.SetCommand("!exit");
                projectionFrame.dispatchEvent(new WindowEvent(projectionFrame, WindowEvent.WINDOW_CLOSING));
                break;
            }
        }
    }
    ...
    // fading alpha effect
    private void DiscoverSFlare(int xC, int yC, float diameter, float intensity, int fadeSteps) {
        ...
        diameter*=sizeModifier;
        fadeSteps +=fadeModifier;
        ...
        for (int i = fadeSteps; i > 0; i--) {
            ...
            if (i == 1) {
                ...
                userTelescope.drawLine((int)(xC - diameter), yC, (int)(xC + diameter), yC);

                userTelescope.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, fade));
                userTelescope.setColor(commandColor);
                userTelescope.drawLine(xC, (int)(yC - diameter), xC, (int)(yC + diameter));
                ...
                userTelescope.setColor(commandColor);
                userTelescope.fillOval((int)(xC - diameter / i / 2), (int)(yC - diameter / i / 2), (int)(diameter / i + 1), (int)(diameter / i + 1));
            } else {
                // Draw the big flares
                // Sideways
                userTelescope.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, fade));
                userTelescope.setColor(commandColor);
                int h = (int)(diameter / (fadeSteps - i) + 1);
                int w = (int)(diameter / i / 2 + 1);
                userTelescope.fillOval(xC - w / 2, yC - h / 2, w, h);
                // Up
                userTelescope.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, fade));
                userTelescope.setColor(commandColor);
                h = (int)(diameter / i / 2 + 1);
                w = (int)(diameter / (fadeSteps - i) + 1);
                userTelescope.fillOval(xC - w / 2, yC - h / 2, w, h);
            }
    ...
    private void DiscoverNebula(int xC, int yC, int width, int height, float intensity, int fadeSteps) {
        ...
        for (int i = fadeSteps; i > 0; i--) {
            ...
            width = (int) (height * sizeModifier * rK4);
            height = (int) (width * sizeModifier * rK4);
